...
function render() {
  ...
  document.querySelector<HTMLDivElement>('#app')!.innerHTML = `
    <div class="app-shell tactical-shell ${drawerExpanded ? 'drawer-expanded' : 'drawer-collapsed'}">
      ...
      <aside class="directions-sheet ${directionsOpen ? 'is-open' : ''}" ${directionsOpen ? '' : 'hidden'}><div class="dialog-top"><div><p class="overline">APPROVED ROUTE · 1.4 km · EST. 18 min</p><h2>Sector 12 to Ward 8 School</h2></div><button type="button" aria-label="${t.close}" data-action="directions-close">×</button></div><ol class="landmark-list"><li><b>1</b><span>Depart Ward 12 Community Hall toward High Ridge Rd.<small>Avoid low culvert.</small></span></li><li><b>2</b><span>Turn right at Post Office Junction.<small>Elevated paved stretch.</small></span></li><li><b>3</b><span>Arrive at Government Higher Secondary School, Ward 8.<small>Use North Gate.</small></span></li></ol><div class="sheet-actions"><button type="button" data-action="offline-card">📥 Save Offline Route Card</button><button type="button" data-action="accessibility">♿ Accessibility Notes</button></div></aside>
      <aside class="assist-sheet ${listenOpen ? 'is-open' : ''}" ${listenOpen ? '' : 'hidden'}><div class="dialog-top"><h2>Listen · TTS preview</h2><button type="button" aria-label="${t.close}" data-action="listen-close">×</button></div><div class="audio-player"><button type="button" data-action="audio-toggle">▶</button><span>Step 1 of 3 · Follow the official route</span><button type="button" data-action="speed">1.0×</button></div></aside>
      <aside class="assist-sheet isl-sheet ${islOpen ? 'is-open' : ''}" ${islOpen ? '' : 'hidden'}><div class="dialog-top"><h2>ISL guidance preview</h2><button type="button" aria-label="${t.close}" data-action="isl-close">×</button></div><div class="isl-video">ISL VIDEO<br /><small>Approved media pending review</small></div><p>Caption: Follow the official route to the safe shelter.</p></aside>
      ${layersOpen ? '<aside class="layers-panel"><strong>Map layers</strong><label><input type="checkbox" data-layer="hazard" checked /> Red Zone (Flood Inundation)</label><label><input type="checkbox" data-layer="route" checked /> Approved Route Vector</label><label><input type="checkbox" data-layer="shelters" checked /> Safe Shelters</label></aside>' : ''}
    </div>`;
  bindInteractions();
  initMap();
}
...
function bindInteractions() {
  ...
  document.querySelector<HTMLButtonElement>('[data-action="route"]')?.addEventListener('click', () => { routeStarted = true; map?.fitBounds([[75.98, 11.48], [76.19, 11.62]], { padding: 120, duration: 1100 }); window.setTimeout(() => { arrivalOpen = true; render(); }, 700); });
  ...
  document.querySelector<HTMLButtonElement>('[data-action="recenter"]')?.addEventListener('click', () => { map?.easeTo({ center: [76.1, 11.55], duration: 600 }); });
  ...
  document.querySelectorAll<HTMLInputElement>('[data-layer]').forEach((input) => input.addEventListener('change', () => { const layer = input.dataset.layer; if (layer === 'hazard') showHazard = input.checked; if (layer === 'route') showRoute = input.checked; if (layer === 'shelters') showShelters = input.checked; map?.setLayoutProperty(layer === 'hazard' ? 'hazard-fill' : layer === 'route' ? 'approved-route' : 'buildings', 'visibility', input.checked ? 'visible' : 'none'); }));
  document.querySelectorAll<HTMLButtonElement>('[data-action^="voice-"]').forEach((button) => button.addEventListener('click', () => { const action = button.dataset.action; if (action === 'voice-safe') map?.easeTo({ center: [76.19, 11.62], zoom: 12, duration: 700 }); if (action === 'voice-alert') map?.fitBounds([[76.00, 11.45], [76.24, 11.67]], { padding: 80, duration: 700 }); if (action === 'voice-route') map?.fitBounds([[75.98, 11.48], [76.19, 11.62]], { padding: 80, duration: 700 }); if (action === 'voice-recenter') map?.easeTo({ center: [76.1, 11.55], duration: 600 }); if (action === 'voice-repeat') showToast('Repeat instruction preview: Follow the official route.'); }));
