...
func TestTransferWithOldDestinationRouteRejected(t *testing.T) {
	...
	body := fmt.Sprintf(`{"type":"TRANSFER","new_facility_id":%q,"new_route_id":"RT-OLD-ZONE","idempotency_key":"old-route-transfer"}`, facB)
...
// contract: routes are optional. (A future policy flag would gate this.)
