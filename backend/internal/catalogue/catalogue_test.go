...
// --- A4: structural validation vs launch acceptance ---

func TestValidateDuplicateScenarioIDAcrossStates(t *testing.T) {
	m := validManifest()
	// Add a second state reusing the first state's scenario ID.
	second := StateEntry{
		StateCode: "TN",
		Name:      "Tamil Nadu",
		Languages: []LanguageSupport{{Language: "ta-IN"}},
		Scenarios: []Scenario{
			{ScenarioID: "SC-KL-SYNTH-1", State: "TN", Evidence: EvidenceSynthetic, ExerciseTime: "2026-09-12T04:00:00Z"},
		},
	}
	m.States = append(m.States, second)
	_, err := Validate(m)
	if err == nil || !strings.Contains(err.Error(), "duplicate scenario id") {
		t.Errorf("err = %v, want duplicate scenario id", err)
	}
}

func TestValidateCrossStateHistoricalReferenceRejected(t *testing.T) {
	m := validManifest()
	// A TN scenario must not borrow KL's historical event as its own evidence.
	second := StateEntry{
		StateCode: "TN",
		Name:      "Tamil Nadu",
		Languages: []LanguageSupport{{Language: "ta-IN"}},
		Scenarios: []Scenario{
			{ScenarioID: "SC-TN-1", State: "TN", Evidence: EvidenceHistorical, HistoricalEventID: "EV-KL-2018", ExerciseTime: "2026-09-12T04:00:00Z"},
		},
	}
	m.States = append(m.States, second)
	_, err := Validate(m)
	if err == nil || !strings.Contains(err.Error(), "owned by state") {
		t.Errorf("err = %v, want cross-state historical reference rejection", err)
	}
}

func TestAssessLaunchInsufficientStates(t *testing.T) {
	// One valid state is structurally sound but far below the launch minimum.
	v, err := Validate(validManifest())
	if err != nil {
		t.Fatal(err)
	}
	launch := v.AssessLaunch()
	if launch.Acceptable() {
		t.Error("single-state catalogue must not be launch-acceptable")
	}
	found := false
	for _, g := range launch.Gaps {
		if g.Kind == "INSUFFICIENT_STATES" {
			found = true
		}
	}
	if !found {
		t.Errorf("gaps = %+v, want INSUFFICIENT_STATES", launch.Gaps)
	}
}

func TestAssessLaunchInsufficientScenariosPerState(t *testing.T) {
	m := validManifest()
	// Drop to one scenario in KL (below the 2-case floor).
	m.States[0].Scenarios = m.States[0].Scenarios[:1]
	v, err := Validate(m)
	if err != nil {
		t.Fatal(err)
	}
	...
	found := false
	for _, g := range launch.Gaps {
		if g.Kind == "INSUFFICIENT_SCENARIOS" {
			found = true
		}
	}
	if !found {
		t.Errorf("gaps = %+v, want INSUFFICIENT_SCENARIOS", launch.Gaps)
	}
}

func TestStructuralValidityIndependentOfLaunchAcceptance(t *testing.T) {
	// A structurally valid but launch-incomplete catalogue still validates:
	// structural validation must not require launch thresholds or completed P6
	// language benchmarks just to store a draft manifest.
	v, err := Validate(validManifest())
	if err != nil {
		t.Fatalf("structural validation failed: %v", err)
	}
	if !v.Acceptable() {
		t.Fatalf("structurally complete single state should have no data gaps: %+v", v.Gaps)
	}
	// Launch assessment is a separate, stricter bar.
	if v.AssessLaunch().Acceptable() {
		t.Error("launch acceptance must require more than one state")
	}
}
