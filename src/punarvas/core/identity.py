...
_PASSWORD_SALT = b"punarvas-prototype-directory-v1"
PROTOTYPE_PASSWORD = os.environ.get("PUNARVAS_PROTOTYPE_PASSWORD", "local-prototype-only")


def _auth_secret() -> bytes:
    env = os.environ.get("PUNARVAS_AUTH_SECRET")
    if env:
        return env.encode("utf-8")
    return b"punarvas-prototype-hmac-not-for-production"
...
def issue_access_token(user: UserContext, ttl_seconds: int = TOKEN_TTL_SECONDS) -> str:
    ...
    payload = {
        ...
        "iss": "punarvas-prototype-directory",
    }
...
def verify_access_token(token: str) -> Optional[UserContext]:
    ...
    if int(payload.get("exp", 0)) < int(time.time()):
        return None
    if payload.get("iss") != "punarvas-prototype-directory":
        return None
    try:
