"""
Sthira Typed Domain Exceptions (PKG-0C).
Normative Reference: rules.md (RUL-001, RUL-013, RUL-017, RUL-029, RUL-035).
"""


class SthiraError(Exception):
    """Base exception for all Sthira domain failures."""
...
class AuthorityBypassError(SthiraError):
...
class MissingEvidenceError(SthiraError):
...
class HardGateBlockedError(SthiraError):
...
class MasterScoreProhibitedError(SthiraError):
...
class OutOfCoverageError(SthiraError):
...
class UnauthorizedGeographyAccessError(SthiraError):
...
class AuditIntegrityError(SthiraError):
...
class CapacityExceededError(SthiraError):
...
class ReservationConflictError(SthiraError):
...
class ApprovalConditionUnmetError(SthiraError):
...
class EntityFrozenByObjectionError(SthiraError):
...
class UnauthorizedActionError(SthiraError):
...
class DefectsBlockCompletionError(SthiraError):
...
class UnservicedUnitHandoverError(SthiraError):
