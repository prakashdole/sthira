...
| **2** | Repair D1: `handleSourceQuarantine` jurisdiction check when no live authz | **OPEN** | Sibling `handleSourceTransition` denies absent/expired authorization; `handleSourceQuarantine` must deny absent live authorization before replay disclosure or mutation. |
| **2** | Repair D2: `reservationPayloadHash` omits `PartySize` & `SnapshotVersion` | **OPEN** | Unambiguous deterministic encoding of all semantically discriminating request fields; changed party size or snapshot version returns 409 conflict; identical replay succeeds. |
