...
| O05 | Who approves routes, how local verification is recorded, mode/closure freshness and conflict escalation. P5 preserves route cancellation tombstones and checks route availability against safe zones without predicting or authorizing routes | Government/local response authority | All operational navigation; explicitly OPEN per user |
| O06 | Basemap/gazetteer/elevation licenses, offline rights, hosting/attribution and redistributable map pack format. P5 delivered the engineering seam, validator, budget checks (<= 50 MiB), and delivery protocol; offline resources currently surface SYNTHETIC_DEMO attribution and LicensePending status. BLOCKED_EXTERNAL on official Survey of India / state basemap licenses | Maps/legal/operations | P5/P8 production maps |
