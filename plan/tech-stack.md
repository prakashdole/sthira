...
| Middle model | Deterministic parser plus optional Azure GPT-4.1 mini explanation; legacy Bedrock adapter | **Proposed first benchmark:** Qwen/Qwen3-4B-Instruct-2507, self-hosted, non-thinking, bounded structured output. No external paid model in target emergency flow |
| Middle serving | API process/client adapters | **Proposed:** vLLM on separate private GPU workers; pinned image/model/tokenizer/quantization; single private API protocol |
| ASR | IndicConformer local adapter enables Hindi/Malayalam | Retain IndicConformer candidate and assets; verify every selected state language and separate English coverage. Python/ONNX/Torch runtime allowed in isolated inference worker |
| TTS | Indic Parler-TTS local runtime; small approved-text WAV cache | Retain candidate; evaluate latency/intelligibility per language; pre-generate approved common audio, synthesize only validated template text |
