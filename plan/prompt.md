...
3. **Exercise Backend**:
   ...
   ```
   *Persistence Check*: Restart backend **without** `STHIRA_EXERCISE_SEED=1` to verify clean reload of stored stays. Initial seed requires `STHIRA_EXERCISE_SEED=1` into a fresh database only; reseeding an existing database fails closed because package seeding is non-idempotent (`PKGDEMO-1:1` already exists in database).
