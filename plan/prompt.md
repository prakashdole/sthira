...
| MONOTONIC | DONE | `CLEAN` | Offline guidance manifest monotonic revision verification & tombstone retention (`offlinedelivery/monotonic.go`, `monotonic_test.go`): `ManifestRevisionTracker` enforces monotonic revision sequencing, rejecting revision regressions (`ErrRevisionRollback`) and unauthorized modifications to identical revisions (`ErrIdenticalRevisionModified`). Tombstone retention guards prevent resurrection of revoked packages (`ErrTombstoneViolated`) or dropping of previously recorded cancelled routes and revoked packages (`ErrTombstoneDropped`). Expiry validated against authoritative time (`ErrManifestExpired`). Verified by 4 unit tests in `monotonic_test.go`, staticcheck rc 0, and gosec rc 0. Recorded in `plan/decisions.md` (D76). |
