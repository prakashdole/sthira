...
| C02 | REOPENED | C01 contract; C04 for voice acceptance | **2026-09-25: see section 0.0; not accepted.** Historical worker claim (superseded): Commit `d0e07d5`: Replaced hardcoded package/jurisdiction/facility/route values with unified EXERCISE_CONFIG (`DEMO-EXERCISE`, `PKGDEMO-1`, `FACDEMO-1`, `SZDEMO-1`, `RTDEMO-1`, `snapshot_version: 1`), segregated illustrative preview as non-reservable, dynamic Haversine distance/duration without fabricated metrics, truthful guidance query with authority/freshness metadata handling, bounded place candidate chips via `/api/v3/places/resolve` (HTTP 409 candidates) with context rebinding, strict stay event contract compliance (`POST /api/v3/reservations/{stay_id}/events` with only `type: ARRIVE` and `idempotency_key`), fail-closed arrival reporting awaiting server HTTP 200, idempotency key reuse across retries, session reconciliation on load/reconnect via `GET /api/v3/reservations/{id}`, zero geofencing (manual GPS activation only, auto-pause on `document.hidden`), cryptographic SHA-256 audio verification with tap-to-play autoplay fallback; verified via 43/43 unit tests passing and clean Vite production build. |
| C03 | REOPENED | C00; serial backend ownership after C01 | **2026-09-25: see section 0.0; not accepted.** Historical worker claim (superseded): Commit `e697794`: Bound approved template digest to `(speech_key, language)` tuple in `ScopedContext` via `TemplateDigestKey` and `TemplateDigest`; updated `readApprovedSpeechKeys` in `backend/internal/store/scoped.go` to store digests by `key + "/" + lang`, detect conflicting active approvals deterministically, and fail closed; updated `Orchestrator.Synthesize` and `stageTemplate` to look up digests by `(speech_key, language)`; backwards-compatible fallback for flat un-delimited keys in legacy test fixtures; verified by 5 new acceptance tests in `backend/internal/store/c03_acceptance_test.go` covering multi-language same key resolution, wrong-language digest mismatch rejection, cross-source digest mismatch rejection, `SnapshotRevalidate` language revocation and drift, row-order independence, and conflicting active approvals fail-closed detection against PostgreSQL 18.6, plus full store, orchestration, contracts, and httpserver test suites passing. |
| C04 | REOPENED | C03; C01/C02 for integrated demo | **2026-09-25: see section 0.0; not accepted.** Historical worker claim (superseded): Commit `6c2e4f0`: Refactored `cmd/sthira` and `cmd/sthira-exercise` to share `httpserver.WireVoicePipeline`; enabled voice process routing (`/api/v3/voice/process`) in `cmd/sthira-exercise`; added `AllowSyntheticTemplates` to `orchestration.PipelineConfig` with process-level isolation (enabled only in exercise mode; production defaults to false and rejects synthetic templates with HTTP 422 `ErrTemplateUnknown`, impervious to request headers/params); updated `seedExercise` in `cmd/sthira-exercise` to seed `place_aliases` (including ambiguous "meppadi" aliases returning HTTP 409 candidate chips) and `approved_translations` matching `ExerciseTemplateRegistry(1, 1)`; verified via 4 acceptance tests in `backend/internal/httpserver/c04_acceptance_test.go` covering full audio/transcript pipeline over HTTP + PostgreSQL with private protocol workers (PLUMBING_ONLY: PASS), process isolation (PASS), ambiguous candidate resolution (PASS), and model specifications integrity (PASS); REAL_INFERENCE honestly recorded as NOT_RUN (BLOCKED_HARDWARE: no GPU cluster, weights unretrieved, no authorized cloud spend); complete evidence and runnable worker startup instructions in `plan/evidence/execution-c04.md`. |
| C05 | REOPENED | C00; resource fix first, final rehearsal after C01–C04 | **2026-09-25: see section 0.0; not accepted.** Historical worker claim (superseded): Resource-safe runner in `scripts/run_demo_rehearsal.sh` (unique task-owned DB without drop-first, fails with code 4 on pre-existing DB without STHIRA_DEMO_REUSE=1, safe identifier validation, clean trap cleanup of owned processes and DB); standalone mock protocol workers in `backend/cmd/mock-workers/main.go`; authoritative live DB schema revision check and subsystem distinction (`database`, `migrations`, `source_activation`, `idp`, `models`) in `backend/internal/httpserver/readiness.go`; 7 acceptance journeys executed with real contracts (Journey 1: live/ready; Journey 2: `/api/v3/voice/process` full pipeline returning actions + WAV audio; Journey 3: HTTP 409 `AMBIGUOUS_PLACE` candidate chips without auto-select; Journey 4: stay reservation, lost-response replay with same idempotency key, process restart readback persistence, strict arrival event; Journey 5: Node-driven proximity evaluation with accuracy <=100m, age <=30s, zero geofencing; Journey 6: 503 fail-closed on absent IdP, text guidance resilience during worker outage, reconnect state reconciliation; Journey 7: synthetic labeling, zero audio retention, docker check, honest scorecard); verified by 4 acceptance tests in `backend/internal/httpserver/c05_acceptance_test.go` (sentinel preservation, low/current/missing schema diagnostics, subsystem distinction, full rehearsal) against PostgreSQL 18.6; `PLUMBING_ONLY: PASS`, `REAL_INFERENCE: NOT_RUN (BLOCKED_HARDWARE)`, `CONTAINER_RUNTIME: NOT_RUN (BLOCKED_NO_DOCKER)` recorded in `plan/evidence/execution-R07.md`. |
