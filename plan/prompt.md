...
| CIRCUIT | DONE | `38202ad` on `CLEAN` | High-load circuit breaker pattern for ML worker upstreams (`circuitbreaker.go`, `circuitbreaker_test.go`): in-memory 3-state circuit breaker (`CLOSED`, `OPEN`, `HALF_OPEN`) defends ASR, Middle Model, and TTS pipelines against cascade failures during GPU surges, model crashes, or inference timeouts. Consecutive failures >= `FailureThreshold` trip to `OPEN`, immediately returning `ErrCircuitOpen` without hitting worker network sockets. After `ResetTimeout`, breaker transitions to `HALF_OPEN` admitting trial probes; probes failing immediately re-trip to `OPEN`, while consecutive successes >= `SuccessThreshold` reset to `CLOSED`. Thread-safe locking under `-race` with mockable clock. Verified by 4 unit tests in `circuitbreaker_test.go`, staticcheck rc 0, and gosec rc 0. Recorded in `plan/decisions.md` (D73). |
| READYDIAG | DONE | `0567196` on `CLEAN` | Subsystem readiness inspection and health breakdown (`readiness.go`, `handlers.go`, `readiness_test.go`): `/health/ready` surfaces structured status reports across critical service dependencies (`database`, `migrations`, `rate_limiter`, `prober`, `memory`). When operational, returns HTTP 200 OK with per-subsystem details (`READY`, `DISABLED`, `SKIPPED`) and memory allocation bounds. When dependencies report failure or prober is unconfigured, returns HTTP 503 `DATA_UNAVAILABLE` (`contracts.ErrDataUnavailable`). Strict internal topology and credential redaction verified. Verified by 5 unit tests in `readiness_test.go` and `server_test.go`, staticcheck rc 0, and gosec rc 0. Recorded in `plan/decisions.md` (D74). |
| CAPVERIFY | DONE | `b9871a2` on `CLEAN` | Disaster alert content integrity and canonical SHA-256 digest verification (`verifier.go`, `verifier_test.go`): `NormalizeAlert` generates deterministic canonical representations for NDMA SACHET CAP feeds by sorting references, normalizing text whitespace, and standardizing polygon coordinates to 6 decimal places (EPSG:4326). `ComputeCanonicalDigest` produces deterministic SHA-256 digests; `VerifyAlertIntegrity` enforces constant-time comparison against expected digests with tamper detection. `AlertDeduplicator` provides thread-safe sliding-window TTL replay protection. Verified by 4 unit tests in `verifier_test.go`, staticcheck rc 0, and gosec rc 0. Recorded in `plan/decisions.md` (D75). |
| MONOTONIC | DONE | `9dc639f` on `CLEAN` | Offline guidance manifest monotonic revision verification & tombstone retention (`offlinedelivery/monotonic.go`, `monotonic_test.go`): `ManifestRevisionTracker` enforces monotonic revision sequencing, rejecting revision regressions (`ErrRevisionRollback`) and unauthorized modifications to identical revisions (`ErrIdenticalRevisionModified`). Tombstone retention guards prevent resurrection of revoked packages (`ErrTombstoneViolated`) or dropping of previously recorded cancelled routes and revoked packages (`ErrTombstoneDropped`). Expiry validated against authoritative time (`ErrManifestExpired`). Verified by 4 unit tests in `monotonic_test.go`, staticcheck rc 0, and gosec rc 0. Recorded in `plan/decisions.md` (D76). |
| CARDGEN | DONE | `2b6854e` on `CLEAN` | Accessible plaintext and markdown offline evacuation card generator and validator (`offlineresources/cardgen.go`, `cardgen_test.go`): `FormatPlaintextCard` produces deterministic 80-column line-wrapped plaintext with authority metadata, emergency 112 guidance, active red zones, safe zones with bed capacity and facilities, approved routes, and official safety instructions. `FormatMarkdownCard` outputs accessible markdown with `tel:112` links and hierarchical structure. `ValidateCardCompleteness` verifies mandatory disaster guidance fields; `VerifyCardTextIntegrity` enforces SHA-256 tamper verification. Verified by 5 unit tests in `cardgen_test.go`, staticcheck rc 0, and gosec rc 0. Recorded in `plan/decisions.md` (D77). |
