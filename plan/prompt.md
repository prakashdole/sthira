...
| P6 | Regional ASR, constrained middle model and TTS | P1 + P4 + P5 | IN_PROGRESS | Integration verified: loopback worker protocol + real HTTP voice endpoints + direct text fallback + synthetic eval (20/20 pass) + Sarvam-30B FP8 vLLM config + JSONL subprocess bridges (BLOCKED under O03/O11) + compressed-audio ffmpeg path; real inference BLOCKED_EXTERNAL on GPU & O03/O11 |
| P7 | Backend security, performance and handoff gate B | P0–P6 acceptance evidence | IN_PROGRESS | Prep/rehearsals verified (fuzz clean, 3 recovery rehearsals pass, loadmodel benchmarks pass, crashtest process tests pass on migrated DB, real k6 smoke against actual Go binary on migrated PostgreSQL, real SPDX SBOMs per module, graceful-shutdown in-flight completion); Gate B remains NOT_READY |
