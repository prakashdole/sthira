...
| C02 | DONE (DEMO BROWSER CONTRACT) | C01 contract; C04 for voice acceptance | Commit `47b452d`: Upgraded `mapActions.ts` and `mapActions.test.ts` to V3 proposal contract (`schema_version: '3.0'`), supporting all action variants (`FOCUS_FEATURE`, `HIGHLIGHT_FEATURE`, `SHOW_CHOICES`, `SHOW_ROUTE`, `FIT_FEATURES`, `ZOOM`, `PAN`, `RECENTER`, `SET_LAYER_VISIBILITY`, `OPEN_PANEL`, `SET_LANGUAGE`) and statuses (`OK`, `CLARIFY`, `UNSUPPORTED`, `DATA_UNAVAILABLE`, `ERROR`). Candidate chips handled dynamically via `onClarify` callback when status is `CLARIFY`. Audio verification enforces exact `byte_size` check without tolerance. Full audio metadata (`audio_b64`, `content_type`, `byte_size`, `checksum_sha256`) stored in `ChatMessage` for replay; removed unauthorized browser `speechSynthesis` fallback. Stay state tracking in `reconcileStayState` tracks `RESERVED` instead of `CONFIRMED`. Removed illustrative-arrival success path in `confirmArrival`, failing closed when no server reservation is found. `visibilitychange` listener stops GPS tracking (`stopTracking()`) and terminates active audio recording/microphone tracks when tab is hidden. 46/46 unit tests passing and clean Vite production build. |
...
| C04 | DONE (PLUMBING & ENTRY POINTS) | C03; C01/C02 for integrated demo | Commits `52ae2ae` (ASR silence handling & non-finite sample checks in Python adapter and Go asrworker), `80414f3` (canonical Sarvam system prompt, 512 max output tokens, wire schema mapping), `3a09887` (stdlib MIME codec canonicalization in contracts & asrworker accepting `audio/webm;codecs=opus`, embedded schema in middleworker, and runnable standalone service entry points in `cmd/asrworker/main.go`, `cmd/middleworker/main.go`, and `cmd/ttsworker/main.go`). Full test suites pass across `asrworker`, `middleworker`, `ttsworker`, `contracts`, and `orchestration`. REAL_INFERENCE honestly recorded as NOT_RUN (BLOCKED_HARDWARE: no GPU cluster, weights unretrieved, no authorized cloud spend); complete evidence and runnable module-aware worker startup instructions updated in `plan/evidence/execution-c04.md`. |
| C05 | DONE (RESOURCE-SAFE RUNNER & REHEARSAL) | C00; resource fix first, final rehearsal after C01–C04 | Commit `871c2de`: Fixed subshell export syntax in `scripts/run_demo_rehearsal.sh`. Rehearsal runner executed successfully against PostgreSQL 18.6 with all 7 acceptance journeys passing (live/ready exercise status, `/api/v3/voice/process` full pipeline returning actions + WAV audio, HTTP 409 `AMBIGUOUS_PLACE` candidate chips without auto-select, stay reservation, lost-response replay with same idempotency key, process restart readback persistence, strict arrival event, Node-driven proximity evaluation with accuracy <=100m, age <=30s, zero geofencing, fail-closed 503, text guidance resilience during worker outage, reconnect state reconciliation, synthetic labeling, zero audio retention, docker check, honest scorecard). `PLUMBING_ONLY: PASS`, `REAL_INFERENCE: NOT_RUN (BLOCKED_HARDWARE)`, `CONTAINER_RUNTIME: NOT_RUN (BLOCKED_NO_DOCKER)` recorded. |
