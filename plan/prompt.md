...
| CARDGEN | DONE | `CLEAN` | Accessible plaintext and markdown offline evacuation card generator and validator (`offlineresources/cardgen.go`, `cardgen_test.go`): `FormatPlaintextCard` produces deterministic 80-column line-wrapped plaintext with authority metadata, emergency 112 guidance, active red zones, safe zones with bed capacity and facilities, approved routes, and official safety instructions. `FormatMarkdownCard` outputs accessible markdown with `tel:112` links and hierarchical structure. `ValidateCardCompleteness` verifies mandatory disaster guidance fields; `VerifyCardTextIntegrity` enforces SHA-256 tamper verification. Verified by 5 unit tests in `cardgen_test.go`, staticcheck rc 0, and gosec rc 0. Recorded in `plan/decisions.md` (D77). |
