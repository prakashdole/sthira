...
### Scope, ownership and cloud stop

- Repository `/Users/apple/Documents/Projects/MonitoringZ`; use uppercase `CLEAN`, never `main`. Baseline is `e8b2010`. Verify branch, HEAD, staged/unstaged diffs and applicable instructions before editing. If checkout changed, reconcile rather than reset it.
- There are 18 uncommitted implementation/test files from the interrupted reviewer session, not executor-verified deliverables. Inspect and preserve them; adopt, correct or narrowly remove only those draft changes as evidence requires. Do not overwrite unrelated work, amend history, push or merge. Executor may make local coherent verified-stage commits under project instructions; record exact IDs. Review/planning agents edit only this prompt unless the user separately authorizes implementation.
- **LOCAL WORK ONLY. Do not start/restart instances, run paid inference, deploy, download weights, change security groups or incur new cloud costs.** Earlier authorization to run the instance has ended: the owner requested shutdown. Last AWS observation was `stopping`, not confirmed `stopped`. A read-only state check is allowed; report unexpected running/stopping state to the owner, never infer shutdown from the scheduled timer.
- Existing instance: `i-01d17e39266c292c2`, region `us-east-2`. All three model directories are already on persistent root EBS `vol-0b4e1d279d21586e2`: Sarvam ~37 GB, ASR ~2.4 GB, TTS ~3.6 GB. Stop preserves them; termination deletes this root volume. Do not terminate, move/delete weights or provision replacements. Public IP must be rediscovered after an authorized restart.
- No secrets, tokens, private keys, recordings, model weights, `.txt` files, binaries or raw test logs in Git. Reuse existing evidence Markdown; do not generate enormous logs or a new reporting system. Temporary prior artifacts under `/tmp/sthira-live-session` may be absent and are not a supported launch mechanism.
- One executor, sequential stages L1→L2→L3→L4 below. Shared contracts make extra parallel agents unnecessary. Use the code-review graph if available, then confirm narrow source/callers; never treat stale graph output as current truth.

### Observed results, not completion claims

Real bounded probes after `e8b2010`: Sarvam generated a valid Hindi zoom action; fabricated-route injection returned UNSUPPORTED; silent WAV returned CLARIFY without actions. Indic Parler generated real Hindi waveform on CUDA and served cached audio at 44,100 Hz. IndicConformer ran real ONNX inference on CPU; GPU ASR was not proved. Synthetic audio upload is not a human microphone/quality test.

Failures before the last unverified drafts: destination requests returned safe-zone IDs instead of facility IDs and were rejected; arrival wording produced SHOW_ROUTE instead of a confirmation panel; full pipeline audio failed at TTS. Latest schema narrowing and digest-forwarding drafts have NOT passed a subsequent complete real-model/browser journey. A rebuilt middle binary was not restarted into the live composition. Do not label the prototype or all C00–C05 complete.

Saved local checks: Python adapter tests 18 passed; middle-worker module tests passed after the final schema draft; contracts/orchestration and focused TTS native-rate checks passed. These do not establish final integration, all supported action variants, real language quality or browser acceptance.

### L1 — Review and finish real executable composition locally

Read changed files and their callers, not the whole repository:

- `backend/internal/asrworker/cmd/asrworker/main.go`: draft calls `LoadModel` before creating worker and takes supported languages from runtime. Verify missing model, failed startup and close/cancellation paths; no implicit stub fallback. Preserve silence/non-finite handling from `52ae2ae`; do not reopen fixed vocabulary work without a reproduction.
- `backend/internal/middleworker/cmd/middleworker/main.go`: draft passes `SarvamLimits()` so the executable really uses the configured 512 output budget. Verify transport receives it; do not claim configuration text proves runtime use.
- `src/sthira_v2/speech_tts_adapter.py`: draft selects device via `STHIRA_TTS_DEVICE` and moves both tokenizer batches to model device. Keep CPU local tests and explicit GPU selection, truthful error on unavailable selected device, offline artifact loading and existing remote-code restrictions.
- `backend/internal/ttsworker/cmd/ttsworker/main.go`, `runtime_adapter.go`, `worker.go`: draft requires `STHIRA_TTS_SYNTHETIC_EXERCISE=1`, pre-generates a finite demo catalog and caches by native sample rate. Review startup deadline/cleanup and partial failure. Preserve cache-only request behavior and exact text/language/source/template/digest/voice identity. Demonstrate actual WAV header, declared settings and cache identity agree; do not relabel 44,100-Hz audio as 16,000 Hz. Missing/unapproved templates fail closed. No arbitrary user/model prose synthesis. Synthetic switch must not be enabled by request content or production defaults.
- Health information must not advertise stub ASR, CPU TTS or verified upstream readiness when those facts were not measured. Report actual configured/observed backend separately from readiness; a config-file digest is not a whole-weight digest. Keep this narrow, with no new observability framework.

Acceptance: local subprocess/HTTP executable tests with controlled protocol adapters (explicitly PLUMBING_ONLY), missing-artifact failure, startup/shutdown, language inventory, applied middle token cap, native-rate cache hit and wrong identity rejection. Keep existing real adapters unchanged where already correct. Produce module-aware commands using supported repository entry points; never assume raw vLLM or Python stdin/stdout adapters implement the private worker HTTP protocol.

### L2 — Finish constrained model and TTS integration contracts

1. Review `middleworker/{client.go,runtime_http.go,wire.go,sarvam_config.go,schema.go,schema/model_output.schema.json}` against `backend/internal/contracts` and frontend action consumers. **Draft schema supports only eight actions, while the existing contract includes additional variants such as HIGHLIGHT_FEATURE, FIT_FEATURES and SET_LAYER_VISIBILITY.** Reconcile all supported variants with the canonical validator; do not silently remove features or loosen extra-field validation. Avoid duplicate schemas drifting independently. Verify enum, required/optional fields, intent/status/action relationships and nullable speech fields through focused table-driven checks.
2. The request-specific guided schema draft narrows request/data versions, evidence IDs, speech keys and ordered facility choices. Verify empty context, no templates, no eligible facilities, valid ordered choices, wrong IDs and maximum counts. Its forced complete choice list must remain compatible with accepted choice limits and semantics; do not make valid context unsatisfiable or invent/truncate authoritative choices silently. Inspect trusted-schema type assertions and schema error propagation. Assert the actual outgoing vLLM request carries this schema and intended limits. Keep independent Go validation; never strip arbitrary thinking/prose and silently treat it as valid JSON.
3. Sarvam system prompt must use known entity evidence IDs (not hashes/package IDs), facility IDs for destination choices, and explicit confirmation panels for arrival/booking/calling. Camera actions require no filler speech. ASR transcribes audio rather than taking system instructions; TTS speaks exact approved template text with a separately identified demo voice description. Do not promise that prompt wording alone enforces semantics.
4. `contracts/scoped.go`: draft permits a known FACILITY alias in `KnownPlaces`. Add a focused regression proving valid same-jurisdiction facility alias accepted; wrong kind/jurisdiction/ID, unknown facility and route collisions still rejected. Do not make all place aliases valid destinations.
5. `orchestration/orchestrator.go`: draft forwards `TemplateSHA256` from the exact language tuple. Verify captured outbound private TTS request carries the approved digest, missing/wrong-language binding prevents TTS call, and response returns verifiable audio/settings. Preserve existing snapshot/revocation validation and source authorization; do not add a flat-key fallback.
6. `backend/cmd/sthira-exercise/main.go`: draft adds Hindi instruction data. Ensure package, templates, facility IDs and all worker demo assets agree. Reseeding existing task DB previously failed because referenced route_versions prevented package deletion: either implement correctly scoped repeat seeding or clearly require a fresh task-owned DB and fail before modifying an existing seed. Never truncate a shared database to fix a demo.

Acceptance: coherent strict schemas, correct context serialization and exact outbound TTS digest tested through existing boundaries; no bypass of production authorization, no fabricated authority and no arrival/reservation write from model actions. Reuse existing tests and add only regressions missing for these drafts.

### L3 — Finish the actual browser contract, preserving its design

Read `frontend/v2/src/main.ts`, `mapActions.ts`, existing journey tests and real Go envelopes. Existing 46 passing tests are not browser acceptance.

- `main.ts` currently reads `envelope.metadata?.freshness || 'CURRENT'`: replace with actual source status/version contract, missing/unknown state unavailable rather than CURRENT. Expired/revoked/offline guidance cannot remain operational navigation.
- Current stored audio metadata lacks complete source/template/language/version binding. Preserve required provenance and invalidate replay on relevant change; checksum/byte-size/content-type/settings validation remains mandatory. Do not play obsolete audio as current guidance after language switch/revocation.
- `formatSpeechKeyText` is a hardcoded rewrite. Show text from the existing authorized resolved template contract; if the envelope lacks it, make the smallest reviewed producer/consumer contract change with tests. Captions and returned audio must describe the same approved text. No browser-TTS substitute and no invented success captions.
- Prove validated 3.0 destination actions show facility choices and arrival opens confirmation only; missing reservation cannot report arrival. Synthetic map labels/routes/instructions must stay visibly illustrative, never operational evidence.
- Preserve explicit consent and cleanup for microphone/GPS, existing session/idempotency safeguards, actual server acknowledgement, and usable touch fallback. Fix only reproduced demo blockers; no UI redesign.

Acceptance locally: serve actual frontend + Go exercise + disposable DB + clearly labelled protocol workers; use browser to type destination/zoom/arrival, inspect visible choices/confirmation, replay audio, simulate missing/stale data and unavailable worker, and verify no false success. Protocol recordings/mocked microphones must be labelled as such. Actual human speech quality waits for L5 authorization.

### L4 — Local integration, compact evidence and stop

Discover existing installed tools; do not install/download large dependencies. Run affected checks once after final changes, repeat only on changes/failures:

- From `backend`: `go test ./internal/contracts ./internal/orchestration ./internal/httpserver ./internal/store -count=1`, `go vet ./...`, `go build ./...`; DB cases require a fresh owned migrated PostgreSQL/PostGIS DB, otherwise explicitly NOT_RUN (not PASS).
- Separately from each `backend/internal/asrworker`, `backend/internal/middleworker`, `backend/internal/ttsworker`: `go test ./... -count=1`, `go vet ./...`, `go build ./...`. Parent module tests do not include nested modules.
- Existing Python environment: `python -m pytest tests/test_v2_real_adapters.py -q` (use its installed interpreter); from `frontend/v2`: `npm test` and `npm run build`.
- Review resource ownership before running `scripts/run_demo_rehearsal.sh`; do not attach to occupied unrelated ports. Use one fresh owned DB and owned ports/processes. Do not rely on leftover `/tmp` tokens, tunnels, binaries or seed data. Stop only processes you own.
- Inspect changed-file formatting/diff and ensure no secret/media/raw log/.txt artifacts staged. Record each coherent verified local commit and any NOT_RUN checks. Update existing C02/C04/C05 evidence and this ledger honestly; historical results remain historical.

Deliver: compact changed behavior, verified revisions/checks, unresolved issues, exact reproducible local launch commands and a ready-to-run paid test checklist. **STOP here. Ask the owner for a bounded paid verification window; do not start AWS to finish acceptance automatically.** Local engineering may finish while REAL_MODEL_BROWSER remains NOT_RUN or PARTIAL. No production or phase completion claim.

### Validated Launch Procedures
...
3. **Exercise Backend**:
   ...
   ```
   *Persistence Check*: Restart backend **without** `STHIRA_EXERCISE_SEED=1` (or rely on safe idempotent seeding) to verify clean reload of stored stays without destructive package operations.
...
5. **Teardown**:
   Terminate owned processes and drop disposable database: `dropdb "$TEST_DB"`.
...
3. **Module-Aware Real Worker Binaries**:
   ...
   - **ASR Worker** (`backend/internal/asrworker/cmd/asrworker`):
     ...
     STHIRA_ASR_ADDR="127.0.0.1:8001" STHIRA_ASR_PYTHON="python3" STHIRA_ASR_ADAPTER="sthira_v2.speech_asr_adapter" ./asrworker
     ```
   ...
   - **Middle Worker** (`backend/internal/middleworker/cmd/middleworker`):
     ...
     STHIRA_MIDDLE_ADDR="127.0.0.1:8002" STHIRA_VLLM_URL="http://127.0.0.1:8000" ./middleworker
     ```
   ...
   - **TTS Worker** (`backend/internal/ttsworker/cmd/ttsworker`):
     ...
     STHIRA_TTS_ADDR="127.0.0.1:8003" STHIRA_TTS_PYTHON="python3" STHIRA_TTS_ADAPTER="sthira_v2.speech_tts_adapter" ./ttsworker
     ```
...
4. **Public Backend / Exercise Wiring**:
   Public backend connects to private worker URLs (`STHIRA_ASR_URL="http://127.0.0.1:8001"`, `STHIRA_MIDDLE_URL="http://127.0.0.1:8002"`, `STHIRA_TTS_URL="http://127.0.0.1:8003"`), not raw adapter or vLLM endpoints.

### L5 — Deferred paid acceptance, only after explicit new owner authorization

This section is a checklist, NOT permission to execute. Agree runtime/cost ceiling and whether to stop immediately afterward or leave a short supervised manual-demo window. Verify AWS state/IP, persistent artifacts and shutdown guard. Use the existing instance and loopback services/private SSH tunnels, never public model endpoints. Reuse loaded models; no blind retry loops or downloads. Preserve compact results locally before stopping, then verify stopped state.

Test the final committed composition, not stale binaries: (1) direct Sarvam strict JSON, ASR real clip and silence, TTS real waveform; (2) typed Hindi destination/zoom/arrival/injection through public backend; (3) synthetic WAV and WebM/Opus through ASR→Sarvam→validated map actions→TTS; (4) actual browser microphone and human listening. Example utterances: “मुझे सुरक्षित स्थान दिखाइए”, “नक्शा बड़ा करो”, “मैं पहुँच गया हूँ”. Destination IDs must be eligible facility IDs, arrival must only ask confirmation, silence must not create commands, fabricated IDs must reject. Confirm actual browser displays choices and plays matching audio; repeat one representative phrase to expose obvious inconsistency without running an unbounded benchmark.

Record per case: expected/actual action, HTTP/pipeline state, stage failure, audio rate/size/hash, latency, exact runtime/revisions and whether input was human or synthetic. No chain-of-thought in action/caption/speech channel. A generated waveform is not intelligibility proof. Human mic/voice evaluation and additional languages require actual evidence; do not label Hindi success as multilingual acceptance. Stop on a repeated unresolved failure rather than burning paid inference, preserve evidence and return remaining fixes to local work. External government/route/stay/license/IdP gates remain unchanged.

---

## 0. ACTIVE recovery instructions — review of 2026-09-24

**Start here. The claim that this playbook is complete is rejected.** Reviewed checkout: uppercase `CLEAN`, `96c2066`; review window `1347c2f..96c2066`. The commit inventory, implementation paths, caller wiring and acceptance reports were inspected; all seven Go modules were tested. This is a bounded acceptance review, not certification that every line or possible defect was exhaustively verified. Preserve the substantial working backend. Do not restart P0–P7 or rewrite the architecture.

This section overrides conflicting DONE labels, acceptance claims and continuation instructions below. The old task descriptions remain requirements, and old handoffs remain historical worker claims, not independent proof. The immediate objective is still the round-two browser/phone demonstration using actual selected models. P8/P9 are not accepted. P10 was executed despite this playbook excluding it; do not continue cleanup or blindly revert that commit. P11/P12 and live government activation remain out of scope.

### 0.0 Current checkpoint — 2026-09-25 review of `e70f512`

**Real-instance results:** [Live inference evidence](evidence/live-inference-2026-09-25.md) supersedes NOT_RUN for the bounded probes only. ASR vocabulary correction `924177d` is tested locally and on real CPU inference; 1-second silence still hallucinated text. Parler generated a real waveform. Sarvam strict action requests exhausted output limits; an expanded prompt returned a fabricated speech key correctly rejected by Go. Complete voice/browser acceptance remains blocked. Follow the evidence's bounded next steps before another paid session; do not reopen already-fixed vocabulary work or claim GPU ASR/production voice approval.

**Live-model check (2026-09-25):** The owner now reports a running instance with all three models uploaded. Follow [live-inference-acceptance.md](live-inference-acceptance.md) for authorized instance probes, model instructions, and laptop/phone manual acceptance. The subsequent real-instance evidence above records the bounded probes; local C02/C04 corrections are not closed by uploaded weights or individual model responses. Instance verification is now requested by the owner; the earlier review-only restriction below does not prohibit those requested checks.
...
#### Next executor: bounded corrections, in this order

Read section 0, applicable root instructions and the exact callers/tests listed here. Work on uppercase `CLEAN` with verified local stage commits, no push/main/history rewrite, no `.txt` changes and no raw logs in Git. Do not call the entire playbook complete after these demo corrections: C06–C09 and the original acceptance gates remain separately tracked.

1. **C05 resource ownership first.** `scripts/run_demo_rehearsal.sh` accepted an already-running server on port 8080 after its own child failed to bind, then created/arrived a reservation in that other server's database. Reproduce with an owned sentinel server/database. Fail before any scenario/mutation requests on a port collision; verify child startup and task-owned server identity, including after restart. Use owned available ports and pass the actual backend target into Vite instead of assuming 8080/5173. Preserve unrelated listeners and rows. Validate database identifiers by rejection, not destructive character stripping; enforce PostgreSQL's byte-length limit and derive admin/child DSNs safely. For `--serve`, keep a healthy pipeline: current worker restart uses a new ephemeral port but backend retains the old URL. Prove the final served endpoint works after the outage test and clean up only owned process groups/resources. Do not run the current runner against an occupied development/deployment port.
2. **C03 exact-language boundary.** Remove production `ScopedContext.TemplateDigest` fallback to `ApprovedTemplateSHA[speechKey]`. Missing `(speech_key, language)` must reject, even if the language appears in `ApprovedSpeechKeys`. Update old test fixtures to tuple keys instead of keeping a test-compatibility bypass in runtime code. Preserve successful multi-language/conflict/revalidation fixes. Prove missing tuple, wrong-language and legacy flat key fail before TTS; correct EN/ML tuples and existing DB tests pass. This is a shared-helper contract defect, not a demonstrated bypass of today's DB resolver, which already emits tuple keys.
3. **C04 real-worker executable path + codec contract.** The shared HTTP wiring is useful, but repository worker packages have no runnable service entry points using the real runtime constructors; `cmd/mock-workers` is fake. Reuse the existing ASR/TTS subprocess runtimes and private HTTP servers, and the middle worker wrapping vLLM. Add only the missing executable composition/config/startup/shutdown necessary to run them. Python speech adapters use stdin/stdout JSON, not `--port` HTTP services; raw vLLM does not implement the private typed worker envelope/health contract. Replace the invalid commands in `execution-c04.md` with tested module-aware build/start instructions; distinguish worker auth from vLLM auth and keep workers private. Missing weights must fail closed, not select stubs. Build/start/probe protocol failure paths without GPU; actual model execution waits for authorized instances. Browser sends `audio/webm;codecs=opus`, rejected by the public allow-list and ASR decoder. Canonicalize and validate MIME/codec parameters consistently at their owning boundary (stdlib MIME parsing is available); preserve byte/duration/decompression limits. Prove actual supported browser recordings decode through the private ASR path, including unsupported MIME/codec rejection and missing transcoder. Do not count the mock worker's acceptance of an empty WAV as recording proof.
4. **C02 browser contract and state.** Own `main.ts`, `mapActions.ts` and affected tests/assets; reuse the current design. Execute the actual Go `schema_version=3.0` validated proposal (current map helper requires `1.0`), with typed allow-listed actions and scenario IDs. Do not just relax validation or ignore its return value. Handle CLARIFY/candidates/unsupported/error explicitly. Read `source_status`/`data_version` from the real envelope; never default missing freshness to CURRENT. After withdrawal/offline/expiry, clear or label stale navigation, stop active route claims and refresh/reconcile on reconnect. Remove hardcoded route fallbacks, fake current timestamps/route instructions/ETA from active guidance; keep any illustrative layer conspicuously separate. Bind chosen facility, route, geometry, version and dates to the same frozen exercise data or actual response. Selecting an ambiguous candidate must affect subsequent context rather than always querying the original package.
5. **C02 mutation and capture lifecycle.** Backend stay state is `RESERVED`, not `CONFIRMED`. Reload must restore it; route reopening must not create a second reservation. Preserve the existing stay until an explicit new selection/transfer succeeds. Persist the full pending operation (key + exact payload + target + dates/version) through uncertainty/reload; today's retry rebuilds dates/party/target under an old key. Renew an expired session without silently losing unresolved operations. Only show server-recorded success after validated acknowledgement/readback. Remove the illustrative-arrival success path. Prevent concurrent/double actions and stale async responses. `visibilitychange` currently changes a flag but never clears the GPS watch; stop capture on hidden/operator navigation/cancel/revocation, reject stale callbacks, and separately clean microphone tracks/timers on cancellation/unsupported recorder creation. No real user location/audio is required for deterministic lifecycle tests.
6. **C02 approved text/audio.** Local `formatSpeechKeyText` rewrites authorized text and ignores template args/version. Use one correctly bound approved caption/template representation; unknown/missing text stays unavailable, never “Official guidance: [key]”. Preserve full audio metadata through replay. Require exact size/hash/type/settings validation on initial playback AND replay (current replay passes base64 alone, missing metadata skips checks, and sizes tolerate four bytes). Do not play if integrity cannot be established. The Listen/replay fallback still uses browser speech synthesis; remove that substitution from the selected-TTS demonstration or segregate it explicitly from model acceptance. Prove tampered/missing metadata rejection and usable text/autoplay-denial fallback.
7. **C05 truthful final rehearsal and diagnostics.** After C02/C04, drive the actual browser choice→reserve→reload→arrival and voice→map→approved caption/audio on laptop and phone viewport. Assert server state and observed responses, not just PASS strings. Keep ordinary same-key replay separate from a genuinely lost response; inject the latter for browser recovery evidence. Node proximity units are not browser/OS GPS lifecycle proof; worker outage is not offline/reconnect proof. `docker info` proves daemon availability only, never container runtime acceptance. Model readiness must distinguish Ready/Warm/stale health and complete configured stages; synthetic OPERATIONAL seed rows are not government activation. Preserve API DB readiness separately from unavailable models/IdP. Current-schema container persistence/backup/restore remains NOT_RUN until exercised. Update R07/round-two-demo evidence with actual results; no universal accepted banner.

**Scheduling:** one worker follows the order above, then real instance acceptance once inputs arrive. If explicitly assigned two workers, use isolated worktrees: A owns C03 + C04/backend/codec/runtime launch; B owns C02/browser. Coordinator owns the C05 resource repair and final integration/ledger; freeze shared contract changes before UI integration. Do not start new mobile work or additional backend features to fill the wait. C09 remains an explicitly assigned bounded security/disposition task, required before claiming production exposure is ready.

**Stop/acceptance:** commit fixes with focused regression evidence. Rerun affected modules, backend on an owned migrated DB, frontend unit/build plus served flows; compiled-out/no-test/skipped checks are not acceptance. Report separately: browser engineering, real model inference, deployment lifecycle, native C06/C07, whole-system C08, C09 and external authority gates. Do not deploy or switch to main. Ask for instance access only when needed for the actual next check; do not infer access from provisioning status.

### 0.1 What was verified, and what failed acceptance

At `96c2066`, the standard backend suite passed on an owned disposable PostgreSQL 18.6 database migrated through 0010: 910 test/subtest PASS events, zero FAIL/SKIP events. These are test events, not 910 independent product requirements. Both `TestCrossProcessLastSpace` and `TestCrashAfterCommitBeforeResponse` executed and passed with `-tags crashtest` and `STHIRA_RUN_PROCESS_TESTS=1`. Standard tests passed separately in asrworker, ttsworker, middleworker, eval, loadmodel and deploy/go-backend/migrate. `go vet ./...` and `go build ./...` passed in all seven Go modules. Browser `npm test`: 43 passed; `npm run build`: passed. Tests using fake inference remain fake inference. No native app build, physical-device acceptance, participant study, actual selected-model inference, container lifecycle, or fresh external security scan was performed by this review.

Targeted temporary probes were run against actual HTTP handlers/PostgreSQL and removed after use. Findings below distinguish these reproductions from source inspection. An initial guidance probe returned an empty list because its fixture body named a different facility; the corrected probe first proved the valid positive control before testing wrong jurisdiction and quarantine. One process-test invocation overlapped removal of a temporary probe file and failed to compile; its clean rerun executed both process tests successfully. Neither invocation issue is a product defect.

| Finding | Evidence at reviewed revision | Required correction |
| --- | --- | --- |
| Guidance ignores authority/scope | Real HTTP+DB: valid jurisdiction returns facility; WRONG-JURISDICTION also returns it; quarantining source still returns it, all HTTP 200. `store/choice.go:Eligible` reads by package ID without enforcing source lifecycle or jurisdiction. | C01; preserve stricter reservation commit guards |
| Ambiguous place cannot be selected | Real HTTP+DB with two aliases returns 409 `AMBIGUOUS_PLACE` but no candidates. `handleResolvePlace` discards `AmbiguousPlaceError` candidates; browser expects `errors[0].details.candidates`. | C01/C02 contract and UI |
| Browser demo does not use exercise data coherently | `frontend/v2/src/main.ts` hardcodes `IN-KL`, `PKG-EXERCISE-01`, `FAC-DEMO-01`, September 12–13 dates and snapshot 1; binary seeds `DEMO-EXERCISE`, `PKGDEMO-1`, `FACDEMO-1`. Errors retain fabricated available capacity, route ID, distances and durations. | C02 |
| Browser arrival falsely succeeds | `confirmArrival` sets success before HTTP response; its `party_size`/`payload` are not in `stayEventRequest`. Actual strict decoder rejects `party_size`. It neither checks status nor reconciles failed/uncertain writes. Route-start also starts GPS without the separate tracking consent action. | C02 |
| Language-specific approval digest lost | Real DB: same speech key approved for en-IN and ml-IN with distinct hashes returns both languages but only the ml-IN digest. `readApprovedSpeechKeys` writes `digests[key]`; `stageTemplate` uses that key for either language. | C03; exact language binding, no weakening approval |
| Exercise voice is not connected | `cmd/sthira-exercise/main.go` never constructs/wires `WithVoiceProcess`; new server defaults to unavailable voice. R07 rehearsal calls proposal validation, not the three-model process path. One-alias lookup is described as ambiguous candidate-chip proof. | C04/C05; real hardware is an additional blocker, not the only missing step |
| Mobile sources are incomplete and disconnected | Android/iOS view models fabricate routes, capacities, reservations, arrival and extension/departure success locally. Android imports missing `DurableOfflineQueue`/`QueueOpType`, calls incompatible shared card constructors; root Gradle settings/version catalog/wrapper and Xcode project are absent. `swiftc -parse` is not type checking/linking. | C06/C07; missing implementation, not only missing hardware |
| Mobile offline trust is not implemented | Shared `OfflineQueue` is an in-memory map; terminal mutation functions lack terminal guards. `MonotonicFreshnessGuard` compares device wall time, has no monotonic elapsed anchor or restart trust reset. Card validator does not verify signed bytes or manifest/card digest binding. | C06; reuse P5 contract, prove native implementation |
| P9 evidence is overstated | `internal/drills/*_test.go` tests local booleans, `MockFacilityLedger`, a test-only handler and a new local hash chain. Android/iOS compatibility tests simulate Go state, not compiled clients. Q02 report asserts 24 participants, voice/screen-reader outcomes without evidence establishing those runs. | C00/C08; retract unsupported observations, do not invent replacements |
| Added helpers are not integrated capabilities | AUDIT has no request-path `Record` caller (probe sees zero events); CIRCUIT constructor has no application caller; CAPVERIFY, MONOTONIC and CARDGEN have no ingestion/publication consumer outside their own helper/tests. | C09; defer unnecessary helpers, do not invent features to justify them |
| Readiness diagnostics misreport schema | `CheckReadiness` compares Go constant `store.SchemaRevision` with literal 10 instead of querying DB for its migrations row. The existing real prober remains a separate protection; the diagnostic subsystem result is still false. | C05 |
| Rehearsal can destroy an existing DB | `scripts/run_demo_rehearsal.sh` drops a fixed/default or caller-named DB before creating it and again in cleanup; interpolates its name into SQL. | C05 before using that runner |

Additional caller-level corrections to verify in the assigned lane: browser text fallback ignores the user's text and sends a canned RECENTER, then presents a summary even on rejection; returned speech keys are replaced with unrelated scenario prose/browser speech synthesis; browser audio playback ignores response integrity metadata. Android foreground-service lifecycle does not establish the required stop-on-hidden-screen behavior merely by omitting background permission. OBS access logs use raw `r.URL.Path`, which includes private resource IDs; the optional limiter is disabled by default, voice-process routes bypass its wrapper, and trusting arbitrary forwarded headers is not proxy authentication. These are not permission to add surveillance, replace model providers, or claim total security.

### 0.2 Execution contract and order

Copy this launch into a new coordinator chat:

> Work in `/Users/apple/Documents/Projects/MonitoringZ`, uppercase CLEAN. Read applicable root instructions, `plan/prompt.md` section 0, sections 1–5 and 8–10, and the original R/B/M/Q requirements relevant to your assigned correction. Execute C00–C05 first; preserve working code and individual local commits. Do not rerun the historical phase plan or treat previous DONE rows as acceptance. Reproduce each failure, repair its responsible layer, and prove the real caller path. Do not add unrelated features, remove safety gates, fabricate evidence, touch `.txt`, push, change main, download weights, incur paid compute, or activate external services. Use owned disposable resources. Continue the next eligible assigned correction after a verified local commit. If hardware blocks actual inference, finish the wiring but keep R07 acceptance blocked. C06–C08 are later production work gated below, not a reason to miss the demo. Report exact remaining blockers once, not invented completion percentages.

One worker: C00 → C05 resource-safety fix → C01 → C02 → C03 → C04 → remaining C05/integrated rehearsal. Stop demo acceptance at missing real inference; do not fill the delay with new backend features. C09 is a bounded disposition/readiness-security cleanup when assigned, not a new platform build. After the demo and accepted Gate B, continue C06 → C07 → C08.

If user assigns parallel workers: use distinct worktrees from the coordinator's integrated SHA; never have two processes commit or run temporary-source probes in one checkout. Two lanes: backend C01→C03→C04; frontend C02 against C01's frozen contract, then C05 runner. Coordinator handles C00 and integration. C05 `httpserver/readiness.go` and C09 overlap backend ownership: schedule serially with that owner. Up to four useful lanes only after contracts are frozen: backend C01/C03, UI C02, exercise wiring C04, operations C05. C04 must await C03's contract integration for final tests. No automatic agent spawning; no extra agents merely to use credits.

Coordinator owns contracts/OpenAPI, shared server composition, migrations, ledger and integration; explicitly hand ownership of those files to one worker at a time. Worker launch: give the common instructions above plus correction ID, worktree, frozen base SHA, owned files and exact next authorized IDs together. A worker must stop at its assignment boundary. Local verified commits only; no push or history rewrite.

### C00 — Correct the evidence and reopen gates (first; coordinator)

Read this review, the section 9 table, `plan/evidence/execution-{R07,B05,M00,M01,M02,M03,M05,Q01,Q02,Q03,Q04,P10}.md`, `plan/drills/drill-report-q02.md`, `plan/decisions.md`, `plan/open-decisions.md`, and `deploy/RELEASE-RUNBOOK.md`. Read targeted portions only.

Separate implementation from acceptance for every affected task. Mark native applications ENGINEERING_REMAINING, not merely BLOCKED_HARDWARE. Mark R02 plumbing separately from actual inference. Withdraw Gate B/P8/P9 release acceptance until its prerequisites are proven. Correct Q02's participant counts/100% success/screen-reader observations to NOT_VERIFIED unless actual authorized study evidence is supplied; retain the earlier claim as explicitly superseded history, never as measured observations. Relabel toy tests as illustrative units or replace them in their owning correction; their green result cannot satisfy production/drill gates. Do not call the project's 7–30-day target a statutory legal requirement while O07 is open. Treat `gov:...` catalogue identifiers as unresolved references unless the source register resolves them to real reviewed documents. Reopen O02/O13 acceptance if D62's mobile selection has no actual two-platform proof; retain the stack as provisional, not a forced rewrite. Do not assert the user approved device/budget choices solely because an agent wrote them.

Record the P10 scope deviation. Inspect remaining imports/build references to deleted files, repair actual broken references if needed, but do not restore 100+ legacy files or repeat cleanup without a concrete need. Reconcile the root geofencing prohibition with the existing narrow foreground-consent override without expanding tracking. Acceptance: no current report, gate summary or task row states a rejected claim as verified; every missing input has an owner and no invented success.

### C01 — Authoritative guidance and usable place resolution (backend; after C00)

Read `backend/internal/store/{choice,scoped,context,source,stay}.go`, `httpserver/stay_handlers.go`, place resolver types, current OpenAPI and their real-DB tests. Preserve the successful R01 safe-zone ordering fix.

At the shared guidance-query boundary, validate package belongs to requested jurisdiction, is current/effective/unexpired/not superseded, and has an operational source with live authorization for that scope. Quarantined/suspended/revoked/retired source data cannot become current eligible destinations. Use existing authority readers instead of a second incomplete validator. Keep public historical display distinct from actionable eligibility; return a truthful stable status. Preserve unknown vs zero capacity, facility membership, route/destination binding and policy order; no invented replacement. Bound requested date ranges before constructing a per-day slice or making per-day DB queries, using the accepted contract/policy; do not let arbitrary multi-century client dates drive work. Recheck reserve/extend/transfer still validate under locks and do not change committed replay semantics.

Return bounded, jurisdiction-scoped actual candidates for an ambiguous place in a single agreed envelope used by browser and native clients. Include stable IDs and actual available disambiguators; do not fabricate names or coordinates. Update OpenAPI and consumer together. An explicit candidate selection must affect the next guidance/context lookup, not just append a chat message.

Acceptance through HTTP+owned DB: valid positive control; wrong jurisdiction with known package ID; quarantine/suspend/expired/superseded package; authorization expiry; unknown/full capacity; missing/closed/wrong-destination route; bounded date input; two same-name places with visible distinct candidates; explicit selection changes context; empty/not-found/error distinct. Read-only queries never mutate capacity. Include withdrawal while UI is open in C02 integration. Do not accept another test that only calls `Eligible` with a mock.

### C02 — Make the existing browser journey real and honest (UI; C01 contract first)

Own `frontend/v2/src/main.ts` and directly affected UI tests/assets. Read R03/R04 requirements, `journey.ts`, `mapActions.ts`, emergency/operator consumers, real Go response shapes and exercise seed. Preserve the visual design; this task is integration, not a redesign.

1. Replace mismatched hardcoded package/jurisdiction/facility/route/version/date values with one server-backed or explicitly frozen exercise configuration shared with C04. Load actual permitted destinations. Treat failure/empty/stale data explicitly; never retain hardcoded 45 beds, verified route or fabricated distance/ETA as a response. If a static preview is kept, segregate it as non-reservable illustrative content, visible in every state.
2. Confirm user choice without silent selection; display actual source/evidence/freshness and authorized route geometry/instructions. Bind pending operations to selected package/version/facility. Keep source revocation/route closure and offline transitions visible and prevent stale navigation/commit claims.
3. Use the actual reservation and stay-event contracts. Remove unsupported ARRIVE fields unless a reviewed additive privacy contract requires them. Store one operation/idempotency key for the same intent through uncertain response/retry; reconcile server state after restart. Never create a fresh key to retry an uncertain mutation. Check status, envelope and IDs before showing success. A local self-report or queued operation is not server-recorded arrival, capacity release or facility confirmation. Handle expired session/stale snapshot/full facility without success badges or discarded original stays.
4. Start GPS only after explicit tracking consent, not automatically on route selection. Stop capture on hidden screen/stop/cancel/permission withdrawal; guard stale callbacks. Proximity remains advisory; explicit confirmation still awaits server acknowledgement. Do not send raw position/accuracy history to an endpoint with no accepted privacy contract.
5. Use C01's actual candidates. Remove fallback that ignores typed text and emits canned action/prose. Missing models must leave usable touch/text lookup and an honest unavailable message. Display/speak the authorized selected output, not unrelated scenario text. Verify returned audio size/hash/content type/settings before playback; handle autoplay denial with a tap. Do not silently substitute browser speech synthesis and call it Indic Parler-TTS; no private speech service fallback.

Acceptance: drive the served UI on laptop and phone viewport against real Go+disposable DB: choice→reserve→reload/readback→explicit arrival; API 400/401/409/503; dropped response with same-key retry; ambiguous location; denied mic/GPS; tab hide; revoked route; offline/no capacity certainty; keyboard/touch and non-map guidance. Unit helpers/build are necessary but insufficient. Use actual browser recording supported by the ASR path; no manually crafted request standing in for the UI journey.

### C03 — Bind approved template digest to language (backend; serial after C01)

Read `store/scoped.go:readApprovedSpeechKeys/SnapshotRevalidate`, `contracts/context.go`, `orchestration/{registry,orchestrator,synthesize}.go`, TTS cache identity and B01 tests. Preserve exact source/version/content approval and migration 0010 quarantine.

Represent and retrieve the digest by speech key AND language (or equivalently select only the explicitly requested language at the trusted resolver, updating all callers). Do not use last-row-wins, shared-text assumptions or a global fallback. Revalidation must check the same source/version/key/language/digest tuple used for synthesis. Detect conflicting active approvals for the same tuple deterministically and fail closed; no arbitrary row choice. Update all serialization, worker context, synthesis-only and cache callers affected by the chosen representation, without leaking credentials.

Acceptance: real DB with same key and different en-IN/ml-IN approved texts; both correct languages synthesize through the actual orchestrator with protocol-test workers; wrong-language/cross-source digest rejects before TTS; changing/revoking either language during slow work affects the correct request; no row-order-dependent result. Real-model voice acceptance remains C04's separate gate.

### C04 — Wire the selected models into the isolated demo (inference; C03 integrated)

Read `cmd/sthira/main.go` and `cmd/sthira-exercise/main.go`, worker binaries/configs, actual adapter loaders, template registry and `plan/evidence/execution-r02.md`. The production command's wiring exists; reuse it at the smallest responsible boundary rather than copy a second large divergent setup. Add only the required exercise composition and validated language/template binding.

The isolated binary must configure the actual three worker clients and persisted context, accept `/api/v3/voice/process`, and drive browser map actions plus returned audio. Synthetic case/phrases must stay conspicuously synthetic and process-isolated. Do not clear `SyntheticOnly` or insert fake government approval to defeat B01. If an exercise-specific permission is necessary, make it explicit and unreachable from ordinary production configuration/request fields, with tests proving isolation. Validate exact selected runtime/revision/license/processor/template requirements: IndicConformer-600M-Multi → Sarvam-30B → Indic Parler-TTS. Check supported codec/language and true audio metadata end to end. Supply runnable worker startup instructions, not comments telling the next person to start unspecified services.

Acceptance has two independent rows: (a) public API/exercise binary + real DB + actual private HTTP protocol workers, labelled PLUMBING_ONLY; (b) actual selected weights executing from browser microphone through all three models on authorized hardware, with model revisions, clip/output hashes, latency and reviewed output, labelled REAL_INFERENCE. For unavailable hardware, finish (a), state exact missing resources, and leave (b)/R07 BLOCKED_HARDWARE. No download, spending or external inference is authorized here. A successful `/voice/commands` proposal validation does not pass either full pipeline row.

### C05 — Safe rehearsal and truthful runtime diagnostics (operations; resource safety before running script)

Read `scripts/run_demo_rehearsal.sh`, `backend/internal/httpserver/{readiness,handlers}.go`, existing DB prober, deploy scripts and recovery evidence. Do not execute the existing destructive rehearsal script unchanged.

Create a unique task-owned database without DROP-first; fail if supplied DB already exists unless an explicitly separate, authorized reuse mode preserves it. Validate names and use safe identifier construction. Track successful ownership creation and only drop that database during cleanup. Preserve existing DBs on startup failure, port collision and interrupt. Derive admin/child connection settings consistently; no silently connecting to a different local server. Own binaries/logs/ports in a temporary directory and clean only owned processes. `--serve` must actually leave the intended demo usable until explicit stop.

Make rehearsal use C01/C02/C04's same case/contracts. Actual browser journeys, HTTP statuses and decoded responses are proof; grepping one known alias is not ambiguous selection, grepping source text is not a GPS drill, HTTP 503 alone is not offline recovery, and proposal validation is not inference. Keep process restart/readback and actual lost-response replay evidence. Publish concise results with REAL_INFERENCE status, not a universal green banner.

Read migration state from the connected DB for readiness diagnostics using the existing prober's authoritative data; missing table/lower revision/query failure cannot display migrations READY. Keep global readiness fail closed. Distinguish API DB readiness from model readiness, source activation and unavailable IdP; do not infer full system readiness from a compile-time constant or configured URL. Recheck container/recovery commands against current revision 10 (and later changes if any); run real container lifecycle only when available, with named-volume persistence across recreation and backup/restore verified. Do not claim that historical revision-9 backup proof covers a changed revision-10 artifact.

Acceptance: pre-existing sentinel DB preserved; failure cleanup preserves unrelated DB/process; unique migrated rehearsal DB created/removed correctly; low/current/missing schema diagnostics through actual HTTP+DB; corrected served UI rehearsal; container run explicitly PASS or NOT_RUN. No external scan or deployment authorization is created.

### C06 — Buildable mobile foundation and actual offline implementation (after demo + accepted Gate B)

Read M00/M01 requirements, `mobile/` build files and all shared/native callers. First reconcile missing build manifests/version catalog/targets and incompatible imports/constructors. Use the provisional KMP/native choice only after the required two-platform proof or explicit user decision; do not select a new framework to dodge repairs. Supply complete reproducible Gradle and Xcode build entry points and required app resources/linkage. Syntax parsing is not a native build.

Implement actual API/session transport and platform-backed private credential storage. Reuse P5 wire contracts/fixtures with strict native decoding and actual signature/digest/manifest-card binding, jurisdiction/version/revocation/resource validation, atomic last-good activation, bounded resume downloads and trust invalidation after restart. Replace the in-memory queue claim with durable platform storage, guarded state transitions and same-key/payload replay reconciliation after process death. `markCommitted` must not revive a terminal failure. Implement monotonic elapsed-time freshness with a process-local server anchor; wall-clock rollback, forward change or restored anchor cannot revive old data. Preserve readable last-known info while denying unverifiable current guidance.

Acceptance must execute the Kotlin/shared implementation and Swift integration, not similarly named Go simulations. Decode real server fixtures in both clients; tamper/cross-jurisdiction/revocation fail; process kill/restart preserves queue and valid stored bytes without restoring freshness trust; interrupted download/replay works through server contracts; tokens remain private. Actual builds required for engineering acceptance, physical devices for P8 acceptance. Missing SDK/device is a named blocker, not a reason to mark incompatible source DONE.

### C07 — Connect both native citizen journeys (C06 accepted)

Read M02–M05 requirements and view models. Remove all local fabricated booking, approval, arrival, extension, departure, route and capacity success. Wire both UIs to C06's real repository/API; preserve honest mock-only previews separately. Selecting Meppadi must query actual candidates, not a hardcoded string branch. Microphone bytes must reach the real voice endpoint and returned audio must be played/validated, not merely counted with “Voice processed”. Implement actual map/resource use and complete accessible non-map fallback.

Use C02's transaction/uncertain-state/consent semantics on both platforms. An Android foreground service can run while the app UI is hidden; the name and missing background permission do not prove foreground-only consent. Stop location on lifecycle hiding/revocation/cancel unless the user explicitly approves a different tracking product. Prove actual OS permission/audio/session/keychain behavior. Native tests/builds/device journeys replace Go imitation as acceptance. M05 stays blocked until both specified physical platform cohorts and accessibility/budget requirements are measured.

### C08 — Rebuild whole-system evidence from real flows (after prerequisites; no invented studies)

Read Q01–Q04 requirements and the corrected C00 evidence. Reuse valid real HTTP/DB/process tests; do not build another mock ledger or test-only server for acceptance. Use the shipped browser/native apps and current schema/artifact for source withdrawal, ambiguity, permission failure, capacity race, uncertain response/restart, route closure, expiry/arrival, offline cold start and rollback. Preserve true unit tests but label their scope accurately.

Run authorized participants/device/language work only when actually available. Record consent, cohort/device/language, tasks, observed failures and measurements in privacy-preserving form; otherwise NOT_RUN, never invented counts/feedback. Resolve historical references to reviewed source artifacts; official-domain-like identifiers and counts alone do not establish history, routes, map rights or language approval. Real-model throughput, resource usage, load/recovery/upgrade tests must identify artifact/hardware/workload and meet agreed targets; smoke p95 and canned synthetic responses cannot establish million-user readiness. B03/B04/M05/Q02/Q03/Q04 remain partial or blocked until their own criteria pass. Stop at Q04; no P10–P12 continuation.

### C09 — Disposition of extra features and bounded security corrections (only assigned, no expansion)

Read D63–D77 and actual call sites before acting. AUDIT, CIRCUIT, CAPVERIFY, MONOTONIC and CARDGEN are helper implementations, not delivered runtime capabilities. Default to DEFERRED / NOT_INTEGRATED and keep them out of acceptance. Do not wire new subsystems merely to make a DONE row true. Preserve existing durable `store.ChainAuditor` and P5 verified mechanisms; do not replace them with duplicate in-memory structures. If a helper is later explicitly required, acceptance must run its consumer and prove the relevant boundary, not just its unit tests.

Specific hazards to resolve before any such integration: auditor slice grows without bound and is not durable; monotonic tracker rechecks only revision/hash under commit lock, not tombstone retention after concurrent validation, and loses state on restart; digest comparison alone does not authenticate a government sender; card generator unconditionally prints “OFFICIAL DISASTER EVACUATION GUIDANCE CARD” for minimally validated input, including synthetic data. These findings are source-level risks in currently unintegrated helpers, not claims that those code paths run in production.

For existing served diagnostics/middleware: sanitize access logs to route patterns rather than private resource IDs/arbitrary paths; keep tokens, transcripts and position out of logs. Make rate-limit documentation truthful (disabled means no protection, not “fail-closed”); cover expensive voice routes if protection is enabled, validate trusted proxy topology, bound IP cardinality and make concurrent shutdown safe. CORS preflight must support actual client headers (including X-Language when used) without opening wildcard credentials. Keep clock-skew header validation distinct from authoritative freshness/replay protection; queued retries cannot be defeated by blindly reusing an old client timestamp. Reuse existing bounded worker admission mechanisms. Check security scanner coverage of the migration module as well as the other six; review suppressions, do not equate clean scanners with end-to-end security. Tests must exercise configured real middleware, including failure responses; do not add another metrics service.

### 0.3 Recovery ledger and stop rule

| Correction | Current status | Depends on | Completion evidence |
| --- | --- | --- | --- |
| C00 | DONE — documentation reconciliation | Current checkout | Initial reconciliation in `18e38f9`, supplemented by the 2026-09-25 review of round-two/C04/R07 claims; current section 0.0 governs. Earlier worker report: All active gate/report contradictions reconciled (drill report Q02 updated to target protocol specifications, Gate B reopened in execution-B05.md, R07 reopened in execution-R07.md, P8 reopened in execution-M05.md, P9 reopened in execution-Q04.md, runbook annotated with C06 dependencies). |
...
| C02 | DONE (LOCAL DEMO BROWSER CONTRACT) | C01 contract; C04 for voice acceptance | Upgraded `mapActions.ts` and `mapActions.test.ts` to V3 proposal contract (`schema_version: '3.0'`), supporting all action variants. Removed unauthorized caption fallback `formatSpeechKeyText`, displaying approved server template text verbatim and setting `backendUnavailable` when missing. Complete audio replay validity enforced in `isAudioValidForReplay` (bound to `data_version`, `language`, `source_version`, `template_version`), invalidated on language change, version change, revocation, expiry, or offline degradation. Cleared pending autoplay on language switch, revocation, and offline state. Stay state tracking in `reconcileStayState` tracks `RESERVED` instead of `CONFIRMED`. Removed illustrative-arrival success path in `confirmArrival`, failing closed when no server reservation is found. `visibilitychange` listener stops GPS tracking (`stopTracking()`) and terminates active audio recording/microphone tracks when tab is hidden. 46/46 unit tests passing and clean Vite production build. |
| C03 | DONE (EXACT-LANGUAGE TUPLE BINDING) | C00; serial backend ownership after C01 | Commit `ef20c4a`: Enforced exact-language tuple binding `(speech_key, language)` in `contracts.ScopedContext.TemplateDigest` without flat fallback; removed legacy fallback bypass; updated all orchestrator and test call sites; verified by 6 acceptance tests in `backend/internal/store/c03_acceptance_test.go` covering multi-language same key resolution, wrong-language digest mismatch rejection, cross-source digest mismatch rejection, `SnapshotRevalidate` language revocation and drift, row-order independence, and conflicting active approvals fail-closed detection against PostgreSQL 18.6, plus full store, orchestration, contracts, and httpserver test suites passing. |
| C04 | DONE (LOCAL PLUMBING, SCHEMAS & ENTRY POINTS) | C03; C01/C02 for integrated demo | All action variants (`FOCUS_FEATURE`, `HIGHLIGHT_FEATURE`, `SHOW_CHOICES`, `SHOW_ROUTE`, `FIT_FEATURES`, `OPEN_PANEL`, `ZOOM`, `PAN`, `RECENTER`, `SET_LANGUAGE`, `SET_LAYER_VISIBILITY`) reconciled in schema and Go contract. `FIT_FEATURES` schema narrowing and contract validation decoupled from `SHOW_CHOICES` (permits known spatial features & routes, does not require eligible destinations, rejects unknown/cross-scope IDs, minItems 1, maxItems 3, uniqueItems). `SHOW_CHOICES` authoritative order and facility restriction preserved. Template text and language-bound template digest forwarding verified. Real worker executable entry points in `cmd/asrworker`, `cmd/middleworker`, `cmd/ttsworker`. Full test suites pass across `asrworker`, `middleworker`, `ttsworker`, `contracts`, `orchestration`, `httpserver`, `store`, and Python adapter (`pytest` 18 passed). REAL_INFERENCE honestly recorded as NOT_RUN (BLOCKED_HARDWARE: instance STOPPED on AWS, no paid compute). |
| C05 | DONE (RESOURCE-SAFE RUNNER, SEEDING & REHEARSAL) | C00; resource fix first, final rehearsal after C01–C04 | Safe demo seeding in `cmd/sthira-exercise/main.go` fails safely without deleting existing bookings, facilities, or stays; normal restart preserves readback. `scripts/run_demo_rehearsal.sh` executes against local PostgreSQL 18.6 with all 7 acceptance journeys passing (live/ready exercise status, `/api/v3/voice/process` full pipeline returning actions + WAV audio, HTTP 409 `AMBIGUOUS_PLACE` candidate chips without auto-select, stay reservation, lost-response replay with same idempotency key, process restart readback persistence, strict arrival event, Node-driven proximity evaluation with accuracy <=100m, age <=30s, zero geofencing, fail-closed 503, text guidance resilience during worker outage, reconnect state reconciliation, synthetic labeling, zero audio retention, docker check, honest scorecard). `PLUMBING_ONLY: PASS`, `REAL_INFERENCE: NOT_RUN (BLOCKED_HARDWARE)`, `CONTAINER_RUNTIME: NOT_RUN (BLOCKED_NO_DOCKER)` recorded. |
| C06 | NOT_STARTED | Demonstration checkpoint and accepted Gate B | Actual native builds and offline execution |
| C07 | NOT_STARTED | C06 | Real API-connected Android/iPhone journeys |
| C08 | NOT_STARTED | C05/C07 and original Q prerequisites | Actual device/language/system acceptance |
| C09 | NOT_STARTED | C00; explicit bounded assignment | Disposition + served security corrections |

Do not close a correction solely because its test file exists or a report has been written. Give each accepted row its actual commit, reproducer, environment, result and remaining external inputs. Run the seven Go modules separately when affected; DB tests require an owned migrated DB and explicit test/admin DSNs. Process tests require both build tag and environment flag. Frontend checks require runtime navigation as well as `npm test`/build. Native checks must build/run native code. Model tests must disclose fake/actual inference. Use bounded temporary logs; never commit raw test streams or secrets.

Next handoff must state: browser demo status; selected-model inference status; backend corrections status; native build/device status; which external O-decisions remain open. A stopped hardware-dependent run is honest partial completion. It is not permission to assert P9 DONE or invent more feature tasks. Resume only assigned eligible work; preserve scope, cost and user authorization.
...
Task statuses below describe this playbook, not whether an older P-phase exists. Section 0 review findings override earlier worker evidence; paragraphs explicitly labelled earlier claims are retained for traceability, not accepted facts. Coordinator updates rows as work happens. Worker reports go in one bounded `plan/evidence/execution-<ID>.md` each; do not duplicate this playbook or create nested progress frameworks.
...
| RLMT | PARTIAL — C09 | `2cc79a9` on `CLEAN` | **2026-09-24 review: Default disabled is not fail-closed; expensive voice routes bypass wrapper.** Earlier worker claim (not acceptance): Per-remote-IP token-bucket rate limiting middleware (`ratelimit.go`, `config.go`, `server.go`): configurable `RateLimitRPS` and `RateLimitBurst`, fail-closed off by default (`EnableRateLimit: false`), exact-match bypass paths (`WithRateLimitBypass`), untrusted `X-Forwarded-For` by default (`WithTrustForwardedFor`), HTTP 429 `RATE_LIMITED` envelope with `Retry-After: 1`, 30-minute stale bucket eviction janitor stopped on server shutdown. Verified by 13 unit/integration tests in `ratelimit_test.go`, full `make check` suite, and clean scanner gate. Recorded in `plan/decisions.md` (D65). |
| HSEC | DONE | `0007371` on `CLEAN` | Comprehensive HTTP security headers & anti-caching middleware (`security_headers.go`, `server.go`): root handler wrapping (`s.withSecurityHeaders(mux)`) applies `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Content-Security-Policy: default-src 'none'; frame-ancestors 'none'`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy: geolocation=(), microphone=(), camera=()`, and `X-XSS-Protection: 0` across all responses (including 404, 405, and 429 rate limit drops). Explicit `no-store, no-cache, must-revalidate, private` and `Pragma: no-cache` applied to sensitive routes (`/api/v3/sessions`, `/api/v3/reservations`, `/api/v3/operations`, `/api/v3/observability/metrics`). Public offline publication routes retain declared cacheability. Verified by 8 endpoint/error test cases in `security_headers_test.go` and clean staticcheck/gosec scans. Recorded in `plan/decisions.md` (D66). |
| ODIAG | DONE | `cf03570` on `CLEAN` | Rate limiter operational diagnostics and uptime metrics exposure (`ratelimit.go`, `observability.go`, `server.go`): `/api/v3/observability/metrics` GET endpoint surfaces `uptime_seconds` (elapsed uptime) and `rate_limiter` diagnostics (`RateLimiterStats`: `enabled`, `rps`, `burst`, `active_ips`, `allowed`, `blocked`). When rate limiting is disabled, endpoint returns `{"enabled": false}`. Thread-safe atomic throughput counters track allowed and blocked request volume; `RateLimiter.Stats()` and `Server.RateLimiterStats()` report active tracked IP bucket counts. Privacy invariants strictly enforced: aggregate integer counters only; zero client IP addresses, mappings, or request paths exposed. Verified by unit and integration tests across enabled/disabled configurations in `observability_test.go` and `ratelimit_test.go`, staticcheck rc 0, and gosec rc 0. Recorded in `plan/decisions.md` (D67). |
| AUDIT | NOT_INTEGRATED — C09 | `796f09f` on `CLEAN` | **2026-09-24 review: No request-path Record calls; separate durable store audit is retained.** Earlier worker claim (not acceptance): Cryptographically chained security audit logger (`auditlog.go`, `server.go`): in-memory append-only `SecurityAuditor` records security-critical HTTP boundary events with monotonic sequencing and SHA-256 hash chaining (`prev_hash -> event_hash`), guaranteeing tamper-evident audit trails. The chain supports runtime verification (`Verify()`) catching payload mutations, hash alterations, and sequence breaks. Zero leakage of bearer tokens, credentials, GPS coordinates, or citizen identities. Verified by 4 unit tests covering sequential hashing, tamper detection, empty input rejection, and concurrent recording under `-race`. Recorded in `plan/decisions.md` (D68). |
| STRICTJSON | DONE | `a4298ab` on `CLEAN` | Uniform strict JSON boundary validation across citizen and guidance endpoints (`stay_handlers.go`, `strict_json_test.go`): input validation unified across `/api/v3/places/resolve`, `/api/v3/guidance/query`, `/api/v3/sessions`, and `/api/v3/reservations`, decoding strictly via `httpjson.DecodeStrict` before probing downstream store or identity dependencies. Strictly enforces single-value JSON, rejection of trailing data (`ErrTrailingData`), bounded nesting depth (`ErrDepthExceeded`), zero duplicate keys (`ErrDuplicateKey`), and unknown field rejection (`ErrUnknownField`). Verified by 4 unit tests in `strict_json_test.go` and zero staticcheck/gosec findings. Recorded in `plan/decisions.md` (D69). |
| CLOCKDRIFT | BOUNDED_HEADER_CHECK_ONLY | `0468220` on `CLEAN` | **2026-09-24 review: Optional untrusted header validation is not authoritative freshness/replay protection.** Earlier worker claim (not acceptance): Client clock drift and stale request protection middleware (`clockdrift.go`, `server.go`, `contracts/errors.go`): state-modifying and guidance HTTP endpoints enforce clock bounds via `s.withClockDriftGuard`. When clients supply `X-Client-Timestamp` (RFC 3339), timestamps > 5 minutes in future and stateful requests (POST/PUT/DELETE/PATCH) > 15 minutes stale reject with HTTP 400 `CLOCK_DRIFT` (`contracts.ErrClockDrift`). Malformed timestamps reject with HTTP 400 `INVALID_VALUE`. Absent timestamps pass transparently for backward compatibility. Verified by 4 unit tests in `clockdrift_test.go`. Recorded in `plan/decisions.md` (D70). |
| CORS | DONE | `141502b` on `CLEAN` | Dynamic CORS defense and allowed origins strict isolation middleware (`cors.go`, `config.go`, `server.go`): `WithAllowedOrigins` / `Config.AllowedOrigins` enforces strict cross-origin isolation. Wildcard origins (`"*"`) are strictly forbidden and filtered. Preflight OPTIONS requests from allowed origins return 204 No Content with `Access-Control-Allow-Origin`, `Access-Control-Allow-Methods`, `Access-Control-Allow-Headers`, `Access-Control-Expose-Headers`, and `Access-Control-Max-Age: 86400`. Preflight requests from disallowed origins return HTTP 403 `FORBIDDEN` (`contracts.ErrForbidden`). Standard cross-origin requests from allowed origins attach `Access-Control-Allow-Origin` and `Vary: Origin`. Requests without `Origin` pass through unmodified. Verified by 4 unit tests in `cors_test.go`, staticcheck rc 0, and gosec rc 0. Recorded in `plan/decisions.md` (D71). |
| EMERGENCY | DONE | `6eb9856` on `CLEAN` | Statutory emergency dialler verification & citizen action validation (`frontend/v2/src/emergency.ts`, `emergency.test.ts`, `main.ts`): emergency dispatch is strictly restricted to approved Indian statutory emergency numbers (`112` ERSS, `108` ambulance, `101` fire, `100` police, `1077` DDMA, `1070` SDMA); non-statutory or foreign numbers reject with `PROHIBITED_UNAUTHORIZED_NUMBER`. Explicit trusted citizen gesture (`event.isTrusted === true`) is strictly required; synthetic/programmatic calls reject with `PROHIBITED_UNTRUSTED_GESTURE`. Background/hidden calls reject with `PROHIBITED_BACKGROUND_EXECUTION`. Local in-memory ring audit trail logs all attempts. Verified by 9 unit tests in `emergency.test.ts` (43/43 frontend tests pass) and production build clean. Recorded in `plan/decisions.md` (D72). |
| CIRCUIT | NOT_INTEGRATED — C09 | `38202ad` on `CLEAN` | **2026-09-24 review: Unit-tested helper has no application constructor/call site.** Earlier worker claim (not acceptance): High-load circuit breaker pattern for ML worker upstreams (`circuitbreaker.go`, `circuitbreaker_test.go`): in-memory 3-state circuit breaker (`CLOSED`, `OPEN`, `HALF_OPEN`) defends ASR, Middle Model, and TTS pipelines against cascade failures during GPU surges, model crashes, or inference timeouts. Consecutive failures >= `FailureThreshold` trip to `OPEN`, immediately returning `ErrCircuitOpen` without hitting worker network sockets. After `ResetTimeout`, breaker transitions to `HALF_OPEN` admitting trial probes; probes failing immediately re-trip to `OPEN`, while consecutive successes >= `SuccessThreshold` reset to `CLOSED`. Thread-safe locking under `-race` with mockable clock. Verified by 4 unit tests in `circuitbreaker_test.go`, staticcheck rc 0, and gosec rc 0. Recorded in `plan/decisions.md` (D73). |
| READYDIAG | REOPENED — C05 | `0567196` on `CLEAN` | **2026-09-24 review: Migration subsystem compares a Go constant with literal 10, not actual DB revision.** Earlier worker claim (not acceptance): Subsystem readiness inspection and health breakdown (`readiness.go`, `handlers.go`, `readiness_test.go`): `/health/ready` surfaces structured status reports across critical service dependencies (`database`, `migrations`, `rate_limiter`, `prober`, `memory`). When operational, returns HTTP 200 OK with per-subsystem details (`READY`, `DISABLED`, `SKIPPED`) and memory allocation bounds. When dependencies report failure or prober is unconfigured, returns HTTP 503 `DATA_UNAVAILABLE` (`contracts.ErrDataUnavailable`). Strict internal topology and credential redaction verified. Verified by 5 unit tests in `readiness_test.go` and `server_test.go`, staticcheck rc 0, and gosec rc 0. Recorded in `plan/decisions.md` (D74). |
| CAPVERIFY | NOT_INTEGRATED — C09 | `b9871a2` on `CLEAN` | **2026-09-24 review: No ingestion consumer; digest comparison alone is not sender authentication.** Earlier worker claim (not acceptance): Disaster alert content integrity and canonical SHA-256 digest verification (`verifier.go`, `verifier_test.go`): `NormalizeAlert` generates deterministic canonical representations for NDMA SACHET CAP feeds by sorting references, normalizing text whitespace, and standardizing polygon coordinates to 6 decimal places (EPSG:4326). `ComputeCanonicalDigest` produces deterministic SHA-256 digests; `VerifyAlertIntegrity` enforces constant-time comparison against expected digests with tamper detection. `AlertDeduplicator` provides thread-safe sliding-window TTL replay protection. Verified by 4 unit tests in `verifier_test.go`, staticcheck rc 0, and gosec rc 0. Recorded in `plan/decisions.md` (D75). |
| MONOTONIC | NOT_INTEGRATED — C09 | `9dc639f` on `CLEAN` | **2026-09-24 review: No publication consumer; in-memory tracker is not durable P5 acceptance.** Earlier worker claim (not acceptance): Offline guidance manifest monotonic revision verification & tombstone retention (`offlinedelivery/monotonic.go`, `monotonic_test.go`): `ManifestRevisionTracker` enforces monotonic revision sequencing, rejecting revision regressions (`ErrRevisionRollback`) and unauthorized modifications to identical revisions (`ErrIdenticalRevisionModified`). Tombstone retention guards prevent resurrection of revoked packages (`ErrTombstoneViolated`) or dropping of previously recorded cancelled routes and revoked packages (`ErrTombstoneDropped`). Expiry validated against authoritative time (`ErrManifestExpired`). Verified by 4 unit tests in `monotonic_test.go`, staticcheck rc 0, and gosec rc 0. Recorded in `plan/decisions.md` (D76). |
| CARDGEN | NOT_INTEGRATED — C09 | `2b6854e` on `CLEAN` | **2026-09-24 review: No delivery consumer; synthetic data must never receive an unqualified OFFICIAL title.** Earlier worker claim (not acceptance): Accessible plaintext and markdown offline evacuation card generator and validator (`offlineresources/cardgen.go`, `cardgen_test.go`): `FormatPlaintextCard` produces deterministic 80-column line-wrapped plaintext with authority metadata, emergency 112 guidance, active red zones, safe zones with bed capacity and facilities, approved routes, and official safety instructions. `FormatMarkdownCard` outputs accessible markdown with `tel:112` links and hierarchical structure. `ValidateCardCompleteness` verifies mandatory disaster guidance fields; `VerifyCardTextIntegrity` enforces SHA-256 tamper verification. Verified by 5 unit tests in `cardgen_test.go`, staticcheck rc 0, and gosec rc 0. Recorded in `plan/decisions.md` (D77). |
